...
function filterHandler(){
  $("body").on("click", ".filter-section ul li", function(){

    //Clear active navigation
    $("#container-filter").find("li").removeClass("filter-active");

    //Set active navigation
    $(this).addClass("filter-active");

    //Get filter and value selected
    var filter = ($(this).parent().siblings("h5").html());
    var val = $(this).html();
...
function generateFilters(filters){
  ...
  $.each(filters, function(i, item) {
    ...
    $.each(item, function(k, value) {
      var li = $("<li>").html(value).appendTo(ul);
    });
  })
}
